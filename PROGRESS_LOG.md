...
| 6  | M6a — Capability/context/key filter (stage 1) + tests | ☐ | hard filtering w/ rejection reasons |
| 7  | M6b — Scoring engine & strategies (stage 2) + tests | ☐ | Draft/Balanced/Premium, configurable weights, reasons |
| 8  | M8a — Key manager + tests | ☐ | priority/health/quota/capability, rotation |
| 9  | M7a — Provider adapters (mock + LiteLLM) + error norm | ☐ | 7 mock behaviors + real Groq/OpenRouter |
| 10 | M7b — Execution engine + tests | ☐ | route→key→adapter→normalize→telemetry |
| 11 | M8b — Failover/retry system + tests | ☐ | bounded, classify, rotate, fallback |
| 12 | M12 — Telemetry/usage recording + tests | ☐ | per-execution metadata, no secrets |
| 13 | API — FastAPI layer + schemas + integration tests | ☐ | /classify /profile /route /execute /providers /health |
| 14 | Backend integration + E2E tests & self-review | ☐ | full pipeline + failure→fallback→success |
| 15 | M9 — Mobile-first frontend (4 screens + state machine) | ☐ | 360/390/430, lifecycle FSM |
| 16 | Frontend tests + mobile viewport verification | ☐ | Vitest/RTL + Playwright |
| 17 | Final full-stack verification & submission evidence | ☐ | E2E, README, exit-gate review |
